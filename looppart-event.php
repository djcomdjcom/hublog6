...
<article class="post-<?php the_ID(); ?> post linkarea style-event row p-4  <?php if (in_category('event-closed')) echo 'closed'; ?>">
    ...
    <?php endif; ?>
    <span class="col-sm-5 p-0 mb-3 mb-sm-0 thumbnail"> <span class="attachment">
    <?php
	...
	<?php get_template_part('cat_icon');//カテゴリーアイコン ?>
      ...
      <span class="todetail"> <span href="<?php if(post_custom('events-page_url')) :?><?php echo(post_custom('events-page_url')) ;?>" target="_blank<?php else :?><?php the_permalink(); ?><?php endif;?>" rel="bookmark" title="<?php the_title(); ?>">詳細・参加方法</span> </span>
